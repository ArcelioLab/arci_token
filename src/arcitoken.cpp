#include "../include/arcitoken.hpp"
#include "eosio.token.cpp"

using eosio::check;

namespace arci {
    void arcitoken::destroy(eosio::name issuer,
        ...
        check(itr->supply.amount == 0, "supply should be 0, retire first");

        statstable.erase(itr);
    }
    
    // void arcitoken::addsupply(eosio::asset addsupply) {
    //     require_auth(_self);
    ...
    void arcitoken::subsupply(eosio::asset subsupply) {
        require_auth(_self);
    ...
    void arcitoken::retire(eosio::name from,
    ...
    void arcitoken::burn(eosio::name from,
                                    eosio::asset quantity,
                                    std::string memo) {
        auto sym = quantity.symbol;
        check( sym.is_valid(), "invalid symbol name" );
    ...
    void arcitoken::pause(eosio::symbol symbol) {
        require_auth(_self);
    ...
    void arcitoken::unpause(eosio::symbol symbol) {
        require_auth(_self);
    ...
    void arcitoken::freeze(eosio::name owner) {
        require_auth(_self);
    ...
    void arcitoken::unfreeze(eosio::name owner) {
        require_auth(_self);
...
EOSIO_DISPATCH(arci::arcitoken,
(create)(open)(close)(issue)(transfer)(retire)(burn)(destroy)(subsupply)(pause)(unpause)(freeze)(unfreeze)
)
